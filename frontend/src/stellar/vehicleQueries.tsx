...
export async function fetchVehicles(): Promise<Vehicle[]> {
    ...
    const txList = await client.list_vehicles();
    const modelNames = (txList.result ?? []) as string[];

    const vehicles: Vehicle[] = [];

    for (const model of modelNames) {
        // 2. Get Details for each Model
        // Returns: [Brand, Availability, Rate, Stock]
        const txVehicle = await client.get_vehicle({ model });
        const data = (txVehicle.result ?? []) as any[];

        if (data.length < 3) continue;

        const brand = data[0] as string;
        const availability = data[1] as string;
        const rateBigInt = data[2] as bigint; // i128
        const rate = Number(rateBigInt); // Assuming rate fits in Number logic for MVP

        const description = "Descripción no disponible en contrato v2"; // Or we add it to contract later

        const { rating, reviews } = generateMockReviews(model);
    ...
    // Fallback Mock Data if Contract is Empty (for Demo)
